...
IKEv2_PACKET_PAYLOAD *Ikev2CreateCPReply(IKEv2_SERVER *ike, IKEv2_CP_PAYLOAD *req)
{
	...
	for (UINT i = 0; i < attrCount; ++i)
	{
		...
		case IKEv2_INTERNAL_IP4_ADDRESS:
			...
			add->value = NewBufFromMemory(IPV4(ip.address), IPV4_SIZE);
		...
		case IKEv2_INTERNAL_IP4_NETMASK:
			...
			add->value = NewBufFromMemory(IPV4(mask.address), IPV4_SIZE);
		...
		case IKEv2_INTERNAL_IP4_DNS:
			...
			IP dns;
			SetIP(&dns, 8, 8, 8, 8);
			add->value = NewBufFromMemory(IPV4(dns.address), IPV4_SIZE);
			ok = true;
			break;
	...
	IP dns;
	SetIP(&dns, 8, 8, 4, 4);
	adda->value = NewBufFromMemory(IPV4(dns.address), IPV4_SIZE);
...
IKEv2_PACKET_PAYLOAD *Ikev2CreateNATNotify(UINT64 SPIi, UINT64 SPIr, IP *ip, UINT port, USHORT type)
{
	...
	if (IsIP6(ip))
	{
		WriteBuf(buf, ip->address, sizeof(ip->address));
	}
	else
	{
		WriteBuf(buf, IPV4(ip->address), IPV4_SIZE);
	}
...
void ProcessIKEv2SAInitExchange(IKEv2_SERVER *ike, IKEv2_PACKET *header, UDPPACKET *p)
{
	...
	if (SAr == NULL)
	{ // All transforms are incompatible, SA not chosen
	...
	if (nat_source_i != NULL && nat_dest_i != NULL)
	{
		...
		if (IsIP6(&p->SrcIP))
		{
			WriteBuf(bSrc, p->SrcIP.address, sizeof(p->SrcIP.address));
		}
		else
		{
			WriteBuf(bSrc, IPV4(p->SrcIP.address), IPV4_SIZE);
		}
		...
		if (IsIP6(&myIP))
		{
			WriteBuf(bDest, myIP.address, sizeof(myIP.address));
		}
		else
		{
			WriteBuf(bDest, IPV4(myIP.address), IPV4_SIZE);
		}
...
IKEv2_PACKET_PAYLOAD *Ikev2CreateTSr(IKEv2_SERVER *ike, IKEv2_TS_PAYLOAD *ask)
{
	...
	IP myIP;
	StrToIP(&myIP, ike->ike_server->Cedar->Server->DDnsClient->CurrentIPv4);
	WriteBuf(ip, IPV4(myIP.address), IPV4_SIZE);
	...
	if (!(pIDi == NULL || pSAi == NULL || pTSi == NULL || pTSr == NULL))
	{
		...
		if (pAUTHi == NULL)
		{
			...
			if (IDr == NULL)
			{
				...
				if (IsIP6(myIP))
				{
					ip_data = NewBufFromMemory(myIP->address, sizeof(myIP->address));
				}
				else
				{
					ip_data = NewBufFromMemory(IPV4(myIP->address), IPV4_SIZE);
				}
		...
		else
		{
			...
			if (IDr == NULL)
			{
				...
				BUF *ip_data = NewBufFromMemory(IPV4(myIP.address), IPV4_SIZE);
			...
			IP sip;
			SetIP(&sip, 10, 10, 10, 10);
			seli->start_address = NewBufFromMemory(IPV4(sip.address), IPV4_SIZE);
			seli->end_address = NewBufFromMemory(IPV4(sip.address), IPV4_SIZE);
			...
			IP rip;
			SetIP(&rip, 0, 0, 0, 0);
			sel->start_address = NewBufFromMemory(IPV4(rip.address), IPV4_SIZE);

			SetIP(&rip, 255, 255, 255, 255);
			sel->end_address = NewBufFromMemory(IPV4(rip.address), IPV4_SIZE);
